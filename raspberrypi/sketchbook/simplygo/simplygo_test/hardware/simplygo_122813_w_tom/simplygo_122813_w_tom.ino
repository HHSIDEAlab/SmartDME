...
{
  SimplyGoPacket serialNumber       =  {PT_SYSTEM, DT_WORD, 2, 110, 0, 0, 0, false  };  //128 bytes total DWORD
  SimplyGoPacket modelName          =  {PT_SYSTEM, DT_BYTE, 2, 110, 0, 0, 0, false  };
  SimplyGoPacket firmwareVersion    =  {PT_LPC, DT_WORD, 41, 0, 0, 0, 0, false  };
  SimplyGoPacket externalVoltage    =  {PT_LPC, DT_WORD, 11, 0, 0, 0, 0, false  };
  SimplyGoPacket externalAmperage   =  {PT_LPC, DT_WORD, 40, 0, 0, 0, 0, false  };
  SimplyGoPacket batteryID          =  {PT_LPC, DT_WORD, 34, 6, 262, 518, 774, false  };
  SimplyGoPacket batTemp            =  {PT_LPC, DT_WORD, 34, 0, 256, 512, 768, false  };
  SimplyGoPacket batTime2E          =  {PT_LPC, DT_WORD, 34, 1, 257, 513, 769, false  };
  SimplyGoPacket batPowerLevel      =  {PT_LPC, DT_WORD, 34, 2, 258, 514, 770, false  };
  SimplyGoPacket batAvgCurrent      =  {PT_LPC, DT_WORD, 34, 3, 259, 515, 771, false  };
  SimplyGoPacket batAvgVolt         =  {PT_LPC, DT_WORD, 34, 4, 260, 516, 772, false  };
  SimplyGoPacket batIdealCap        =  {PT_LPC, DT_WORD, 34, 9, 265, 521, 777, false  };
  SimplyGoPacket batActualCap       =  {PT_LPC, DT_WORD, 34, 10, 266, 522, 778, false  };
  SimplyGoPacket batBits            =  {PT_LPC, DT_WORD, 34, 15, 271, 527, 783, false  };
} 
SimplyGoData;
...
void loop() 
{
  ...
  case STATE_WAITING_TO_CAPTURE:
    {
      ...
      if (!lah)
      {
        Debug_println("state wating_to_capture");
        lah = 1;
      }
  ...
  case STATE_FOUND_SIGNATURE_LOW:
    {
      ...
      if (!lah)
      {
        Debug_println("state found_signature_low");
        lah = 1;
      }
  ...
  case STATE_PROCESSING:
    {
      Debug_println("state_processing");
      ...
      //else
        ...
        {
          Debug_print("Error in Processing: ");
          Debug_println("");
        }    
...
bool processPacket(byte *inBuff){
  ...
  // Battery ID
  if (sgData.batteryID.modulo1 == mbo || sgData.batteryID.modulo2 == mbo || sgData.batteryID.modulo3 == mbo || sgData.batteryID.modulo4 == mbo)
  {
  ...
  // Battery Temp
  if (sgData.batTemp.modulo1 == mbo || sgData.batTemp.modulo2 == mbo || sgData.batTemp.modulo3 == mbo || sgData.batTemp.modulo4 == mbo)
  {
    sgData.batTemp.wvalue = inBuffAsWordArray[sgData.batTemp.offset];
    Serial.print("Battery Temp: ");
    Serial.println(sgData.batTemp.wvalue, DEC);
  }
  
  // Time to Empty
  if (sgData.batTime2E.modulo1 == mbo || sgData.batTime2E.modulo2 == mbo || sgData.batTime2E.modulo3 == mbo || sgData.batTime2E.modulo4 == mbo)
  {
    sgData.batTime2E.wvalue = inBuffAsWordArray[sgData.batTime2E.offset];
    Serial.print("Battery Time to Empty: ");
    Serial.println(sgData.batTime2E.wvalue, DEC);
  }
  // Power Level
  if (sgData.batPowerLevel.modulo1 == mbo || sgData.batPowerLevel.modulo2 == mbo || sgData.batPowerLevel.modulo3 == mbo || sgData.batPowerLevel.modulo4 == mbo)
  {
    sgData.batPowerLevel.wvalue = inBuffAsWordArray[sgData.batPowerLevel.offset];
    Serial.print("Battery Power Level: ");
    Serial.println(sgData.batPowerLevel.wvalue, DEC);
  }

  // Battery Average Current
  if (sgData.batAvgCurrent.modulo1 == mbo || sgData.batAvgCurrent.modulo2 == mbo || sgData.batAvgCurrent.modulo3 == mbo || sgData.batAvgCurrent.modulo4 == mbo)
  {
    sgData.batAvgCurrent.wvalue = inBuffAsWordArray[sgData.batAvgCurrent.offset];
    Serial.print("Battery Average Current: ");
    Serial.println(sgData.batAvgCurrent.wvalue, DEC);
  }  
  Average Voltage
  if (sgData.batAvgVolt.modulo1 == mbo || sgData.batAvgVolt.modulo2 == mbo || sgData.batAvgVolt.modulo3 == mbo || sgData.batAvgVolt.modulo4 == mbo)
  {
    sgData.batAvgVolt.wvalue = inBuffAsWordArray[sgData.batAvgVolt.offset];
    Serial.print("Battery Average Voltage: ");
    Serial.println(sgData.batAvgVolt.wvalue, DEC);
  }
  // Ideal Capacity
  if (sgData.batIdealCap.modulo1 == mbo || sgData.batIdealCap.modulo2 == mbo || sgData.batIdealCap.modulo3 == mbo || sgData.batIdealCap.modulo4 == mbo)
  {
    sgData.batIdealCap.wvalue = inBuffAsWordArray[sgData.batIdealCap.offset];
    Serial.print("Battery Ideal Capacity: ");
    Serial.println(sgData.batIdealCap.wvalue, DEC);
  }  
  // Actual Capacity
  if (sgData.batActualCap.modulo1 == mbo || sgData.batActualCap.modulo2 == mbo || sgData.batActualCap.modulo3 == mbo || sgData.batActualCap.modulo4 == mbo)
  {
    sgData.batActualCap.wvalue = inBuffAsWordArray[sgData.batActualCap.offset];
    Serial.print("Battery Actual Capacity: ");
    Serial.println(sgData.batActualCap.wvalue, DEC);
  }  
  // Battery BITS
  if (sgData.batBits.modulo1 == mbo || sgData.batBits.modulo2 == mbo || sgData.batBits.modulo3 == mbo || sgData.batBits.modulo4 == mbo)
  {
    sgData.batBits.wvalue = inBuffAsWordArray[sgData.batBits.offset];
    Serial.print("Battery Bits: ");
    Serial.println(sgData.batBits.wvalue, DEC);
  }  
  ...
  Debug_println(*(word*)sgData.serialNumber.value);
  Debug_print("Model Name: ");
  Debug_println((char*)sgData.modelName.value);
#endif //DISABLE 
